...
typedef struct whiteg_susp_struct {
    snd_susp_node susp;
    long terminate_cnt;
} whiteg_susp_node, *whiteg_susp_type;

void whiteg__fetch(snd_susp_type a_susp, snd_list_type snd_list)
{
    ...
    falloc_sample_block(out, "whiteg__fetch");
    out_ptr = out->samples;
    snd_list->block = out;
    ...
    /* don't overflow the output sample block: */
        ...
        out_ptr_reg = out_ptr;
        if (n)
            RandomFillNormalFloat(out_ptr_reg, n, 0.f, (float)(0.5 * sqrt(1.0 / 3)));
...
void whiteg_print_tree(snd_susp_type a_susp, int n)
{
}
...
sound_type snd_whiteg(time_type t0, rate_type sr, time_type d)
{
    return snd_make_whiteg(t0, sr, d);
}
